...
<table class="table">
	...
	if(isset($_GET['tanggal'])){
		...
		$user = $_SESSION['uname'];
		$brg=mysqli_query($con,"select * from barang_laku where nama_sales ='$user' ORDER by id desc ");
	}
	$no=1;
	...
	<tr>
		...
		<td colspan="7">Omset Sales <?php echo $_SESSION['uname']?></td>
		<?php
		if(isset($_GET['tanggal'])){
			$tanggal=mysqli_real_escape_string($con,$_GET['tanggal']);
			...
			$x=mysqli_query($con,"select sum(total_harga) as total from barang_laku where nama_sales = '$sales'");
			$xx=mysqli_fetch_array($x);
			echo "<td><b> Rp.". number_format($xx['total']).",-</b></td>";
		}
	...
	<!-- <tr>
		...
		if(isset($_GET['tanggal'])){
			...
			$x=mysqli_query($con,"select sum(laba) as total from barang_laku where nama_sales = '$sales'");
			$xx=mysqli_fetch_array($x);
			echo "<td><b> Rp.". number_format($xx['total']).",-</b></td>";
		}
					...
					<form action="barang_laku_act.php" method="post">
						...
						<div class="form-group">
							...
							<select class="form-control" name="nama_pelanggan">
								<?php
								$brg=mysqli_query($con,"select * from data_pelanggan");
								while($b=mysqli_fetch_array($brg)){
									?>
									<option value="<?php echo $b['nama_pelanggan']; ?>"><?php echo $b['nama_pelanggan'] ?></option>
									<?php
								}
								?>
							</select>
